...
public class AuthServices(IConfiguration config) : IAuthServices
{
    ...
    private static Claim? GetClaimType(IEnumerable<Claim> claim, string key)
    {
    ...
    public string? GenerateJwt(List<Claim> claims, EnTokenMode tokenType)
    {
        JwtSecurityTokenHandler tokenHandler = new JwtSecurityTokenHandler();
        string? key = config.GetConnectionString("credentials:key");
        string? issuer = config.GetConnectionString("credentials:Issuer");
        string? audience = config.GetConnectionString("credentials:Audience");
        ...
        SecurityTokenDescriptor tokenDescip = new SecurityTokenDescriptor
        {
            ...
            Expires = General.GenerateDateTime(tokenType),
            Issuer = issuer,
            Audience = audience,
